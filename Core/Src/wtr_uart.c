...
// void limit_range(float *src, float _minValue, float _maxValue){
//     if((*src) < _minValue ) (*src) = _minValue;
//     if((*src) > _maxValue ) (*src) = _maxValue;
// }
...
void UART6Decode(){
    for (int i = 0; i < BAG_LENGTH; i++) {
		...
		if (tempBag.head[0] == header[0] &&
		    ...
		    //麻烦的写法
		    // recvMsg.yaw = tempBag.yaw;
            // recvMsg.left = tempBag.left;
            // recvMsg.mid = tempBag.mid;
            // recvMsg.right = tempBag.right;

           recvMsg = *(RecvMsg_t*)(void*)(tempBag.payload);// 方便的写法 试过可以用
